...
<template>
  <div class="flex flex-col max-w-screen-lg container py-20">
    ...
    <div class="flex flex-col mb-6">
      <h2 class="text-4xl mb-4 font-semibold">{{ data?.name }}</h2>
      <div class="flex gap-4 text-lg mb-6">
        <div class="flex items-center gap-1">
          <Icon name="mdi:clock-time-eight-outline" class="text-yellow-500" />
          <span>{{ data?.cookTimeMinutes }}</span>
        </div>
        <div class="flex items-center gap-1">
          <Icon name="mdi:fire" class="text-yellow-500" />
      ...
      <ul class="grid grid-cols-1 md:grid-cols-2 gap-2 text-lg">
        <li v-for="ingredient in data?.ingredients">
          <label class="flex gap-2 items-center">
            ...
            <div
              class="relative w-6 h-6 rounded-full border-2 border-yellow-500 flex items-center justify-center peer-checked:after:absolute peer-checked:after:w-4 peer-checked:after:h-4 peer-checked:after:bg-yellow-500 peer-checked:after:rounded-full"
            ></div>
